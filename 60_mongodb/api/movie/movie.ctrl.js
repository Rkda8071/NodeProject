...
// 목록조회 (localhost:3000/api/movie?limit=2)
// - 성공 :
...
// - 성공 : 입력된 director, title값으로 객체를 만들고 db추가
//          배열 추가(201 : Created)
// - 실패 : director, title 값 누락시 (400 : Bad Request)
...
const update = (req, res) => {
    ...
    movieModel.findByIdAndUpdate(id, { director, title, year }, { new: true },
        (err, result) => {
            if (err) return res.status(500).end();
            if (!result) return res.status(404).end();
            res.json(result);
...
// 삭제 (DELETE localhost:3000/api/movie/:id)
